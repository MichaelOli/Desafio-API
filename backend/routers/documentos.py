from backend.schemas.documento import DocumentoResposta, DocumentoLista, DocumentoAtualizar
...
@router.post("/upload", response_model=DocumentoResposta, status_code=status.HTTP_201_CREATED)
...
async def listar_documentos(
    ...
    """
    documentos = db.query(DocumentoTexto).filter(
        DocumentoTexto.usuario_id == usuario_atual.id
    ).offset(pular).limit(limite).all()
